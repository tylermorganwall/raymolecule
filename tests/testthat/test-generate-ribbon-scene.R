...
make_ribbon_hetero_model = function() {
  ...
  file = write_pdb_fixture(c(
    chain_a_1$lines,
    chain_a_2$lines,
    format_atom_line(50, "ZN", "ZN", "A", 101, 0.8, 1.2, 0.1, "ZN", record = "HETATM"),
    format_atom_line(51, "O", "HOH", "A", 201, 2.0, -1.0, 0.0, "O", record = "HETATM"),
    format_ter_line(19, "GLY", "A", 2),
    "END"
  ))
  on.exit(unlink(file), add = TRUE)
...
make_ribbon_phosphate_model = function() {
  ...
  file = write_pdb_fixture(c(
    chain_a_1$lines,
    chain_a_2$lines,
    format_atom_line(60, "P", "PO4", "A", 201, 2.5, 1.8, 0.0, "P", record = "HETATM"),
    format_atom_line(61, "O1", "PO4", "A", 201, 3.4, 1.8, 0.0, "O", record = "HETATM"),
    format_atom_line(62, "O2", "PO4", "A", 201, 2.1, 2.6, 0.2, "O", record = "HETATM"),
    format_atom_line(63, "O3", "PO4", "A", 201, 2.1, 1.0, -0.2, "O", record = "HETATM"),
    format_atom_line(64, "O4", "PO4", "A", 201, 1.8, 1.9, 0.8, "O", record = "HETATM"),
...
test_that("ribbon scenes omit explicit vertex normals by default", {
  ...
  expect_true(all(unlist(lapply(scene$shapes, function(x) !x$has_vertex_normals))))
...
test_that("render_model forwards explicit lookat and lookfrom values", {
  model = make_two_chain_model()
  path_scene = generate_ribbon_scene(model, pathtrace = TRUE, subdivisions = 2)
  raster_scene = generate_ribbon_scene(model, pathtrace = FALSE, subdivisions = 2)
  pathtrace_args = NULL
