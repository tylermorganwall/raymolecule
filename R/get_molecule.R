...
get_molecule = function(molecule) {
	...
	if (is.numeric(molecule)) {
		#assume it is a CID
		url = sprintf(
			"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/%s/SDF/?record_type=3d",
			molecule
		)
	} else if (is.character(molecule)) {
		#assume it is a compound name
		url = sprintf(
		...
		stop(
			"Provide a quoted compound name (character) or an unquoted compound ID (numeric)"
		)
	}
	...
	if (status != 200) {
		if (is.numeric(molecule)) {
			#assume it is a CID
			url = sprintf(
				"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/%s/SDF",
				molecule
			)
		} else if (is.character(molecule)) {
			#assume it is a compound name
	...
	if (status == 200) {
		...
		stop(sprintf(
			"Cannot find compound '%s'. Check the compound name or compound ID (CID)",
			molecule
		))
	}
	return(molecule_sdf)
